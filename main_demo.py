        ...
        if res[3] == 1 and res[4] == 0:
            ...
            try:
                ...
                if cls == 0 and score > 0.5:
                    count += 1
                    show(count)
                    sql2 = "select new_obj from info order by id DESC limit 1;"
                    cur.execute(sql2)
                    res1 = cur.fetchone()
                    print("res1:", res1)
                    if res1[0] == 0:
                        count = 1
                        show(count)
                        sql3 = "update info set result=1,count=1,status2=1,new_obj=1 order by id desc limit 1;"
                        cur.execute(sql3)
                        db.commit()
                        totle_count += 1
                        show2(totle_count)
                        ...
                        # 吹砂干净，修改数据库中的结果
                        sql = "update info set result=1,count={},status2=1,new_obj=1 order by id desc limit 1;".format(count)
                        cur.execute(sql)
                        db.commit()
                        totle_count += 1
                        show2(totle_count)
                    ...
                    print("没有修改前count:", count)
                    # 表示没有吹干净，还要继续吹，不修改数据库中的结果的值
                    count += 1
                    sql2 = "select new_obj from info order by id DESC limit 1;"
                    cur.execute(sql2)
                    res1 = cur.fetchone()
                    ...
                    if res1[0] == 0:
                        count = 1
                        show(count)
                        sql3 = "update info set new_obj=1 order by id desc limit 1;"
                        cur.execute(sql3)
                        db.commit()
                    else:
                        print("修改后的count:", count)
                        show(count)
                        print("没有吹砂干净")
                    sql = "update info set result=0,count={},status2=1 order by id desc limit 1;".format(count)
                    cur.execute(sql)
                    db.commit()
                    # time.sleep(2)
                    # if count > 5:
